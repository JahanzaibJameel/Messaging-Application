...
const createMockWebSocket = () => {
  ...
  const simulateMessage = (data: any) => {
    ...
    const messageCallback = mockWebSocket.addEventListener.mock.calls
  ...
  const simulateClose = () => {
    ...
    const closeCallback = mockWebSocket.addEventListener.mock.calls
...
describe('Integration Tests: Store + Chat Service + MMKV', () => {
  ...
  describe('Full Send/Receive Cycle', () => {
    it('should complete end-to-end message flow', async () => {
      ...
      render(
        <TestComponent initialChats={testChats} onMessageSent={onMessageSent} />
      );
  ...
  describe('Data Persistence', () => {
    it('should persist and restore chats across sessions', async () => {
      ...
      expect(mockChatsStorage.set).toHaveBeenCalledWith(
        'chats',
