  ...
  const setStoreContext = useCallback(() => {
    try {
      const storeState = {
        ...
        messages: {
          totalMessages: Object.values(getMessagesByChatId()).reduce(
...
export const withSentryLogging = <P extends object>(WrappedComponent: React.ComponentType<P>) => {
  ...
  const WithSentryLogging = React.forwardRef<any, P>((props, ref) => {
    ...
    return <WrappedComponent ref={ref} {...props} {...loggedProps} />;
  });

  WithSentryLogging.displayName = `withSentryLogging(${wrappedComponentName})`;
