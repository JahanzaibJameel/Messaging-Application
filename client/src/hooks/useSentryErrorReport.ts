...
export const useSentryErrorReport = () => {
  ...
  const reportMessage = useCallback((message: string, level: ErrorReportOptions['level'] = 'info', data?: Record<string, any>) => {
  ...
  const reportUserFeedback = useCallback((feedback: string, context?: {
    screen?: string;
    feature?: string;
    userEmail?: string;
  }) => {
  ...
  const reportPerformanceIssue = useCallback((issue: string, metrics?: {
    duration?: number;
    memoryUsage?: number;
    operation?: string;
  }) => {
  ...
  const reportNetworkError = useCallback((error: Error | string, context?: {
    url?: string;
    method?: string;
    statusCode?: number;
    responseTime?: number;
  ...
  const reportUIIssue = useCallback((issue: string, context?: {
    component?: string;
    action?: string;
    userAction?: string;
  }) => {
