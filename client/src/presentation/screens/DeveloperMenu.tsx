...
import { useFeatureFlags, useFeatureFlagActions, useFeatureFlagsLoading } from '../../hooks/useFeatureFlag';
...
const DeveloperMenu: React.FC = () => {
  ...
  const groupedFlags = Object.entries(DEFAULT_FEATURE_FLAGS).reduce((acc, [key, flag]) => {
  ...
  useEffect(() => {
    ...
    const initializeStore = async () => {
      const { initializeFlags } = require('../../stores/featureFlagsStore').useFeatureFlagsStore.getState();
  ...
  const handleResetOverrides = () => {
    Alert.alert(
      'Reset Overrides',
      'Are you sure you want to reset all developer overrides?',
      [
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'ui': return '#007AFF';
      case 'functionality': return '#34C759';
      case 'experimental': return '#FF9500';
      case 'performance': return '#FF3B30';
      ...
      <View key={key} style={styles.flagItem}>
        <View style={styles.flagContent}>
          ...
          <View style={styles.flagMeta}>
            ...
            {hasOverride && (
              <Text style={[styles.flagMetaText, styles.overrideText]}>
                Override Applied
              </Text>
            )}
          </View>
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={[styles.actionButton, styles.dangerButton]} onPress={handleResetToDefaults}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statsContainer}>
            ...
            <View style={styles.statItem}>
              <Text style={styles.statValue}>
                {Math.round((Object.values(flags).filter(Boolean).length / Object.keys(DEFAULT_FEATURE_FLAGS).length) * 100)}%
              </Text>
