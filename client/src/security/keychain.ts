...
export const getToken = async (): Promise<TokenStorage> => {
  try {
    ...
    const result: TokenStorage = {
      accessToken: accessToken && accessToken.username === ACCESS_TOKEN_KEY ? accessToken.password : null,
      refreshToken: refreshToken && refreshToken.username === REFRESH_TOKEN_KEY ? refreshToken.password : null,
    };
...
export const setToken = async (accessToken: string, refreshToken?: string): Promise<boolean> => {
  try {
    ...
    // Store access token
    const accessResult = await Keychain.setGenericPassword(
      ACCESS_TOKEN_KEY,
      accessToken,
    ...
    if (refreshToken) {
      refreshResult = await Keychain.setGenericPassword(
        REFRESH_TOKEN_KEY,
        refreshToken,
        { service: KEYCHAIN_SERVICE }
      );
