...
export const useFeatureFlagsStore = create<FeatureFlagsStore>((set, get) => {
  ...
  const loadOverrides = (): Record<string, boolean | undefined> => {
    ...
    try {
      ...
      console.warn('Failed to load developer overrides:', error);
    }
  ...
  return {
    ...
    setOverride: (key: string, value: boolean | undefined) => {
      ...
      try {
        ...
        console.warn('Failed to persist developer overrides:', error);
      }
    ...
    resetOverrides: () => {
      try {
        ...
        console.warn('Failed to clear developer overrides:', error);
      }
    ...
    resetToDefaults: () => {
      ...
      try {
        ...
        console.warn('Failed to clear feature flags storage:', error);
      }
...
export const useAllFeatureFlags = () => 
  useFeatureFlagsStore(state => state.getAllFlags());
